...
description: Common Sirius install problems — polkit authentication, disks that don't show up, and RAM requirements.
sidebar:
  order: 5
---
...
[Overview → privilege model](/sirius/overview/#privilege-model)). `pkexec`
...
The storage page's disk dropdown filters out disks that are currently in
...
(see [Diagnostics → require vs. warn](/sirius/diagnostics/#require-vs-warn)),
...
image-copy tmpdir is configured during the live boot — not done yet.
:::
